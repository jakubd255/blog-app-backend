...
public interface PostRepository extends JpaRepository<Post, Integer> {
    @Query("SELECT p.id AS id, p.title AS title, p.date AS date, p.user AS user FROM Post p WHERE p.status = 'PUBLISHED'")
    List<PostSummary> findAllPublishedPostSummaries();

    @Query("SELECT p.id AS id, p.title AS title, p.date AS date, p.status AS status, p.user AS user FROM Post p")
    List<PostDetailedSummary> findAllPostSummaries();
}
